...
| `GET /v2/dashboard/territories/bootstrap` | Leadership dashboard cascade filters — same JSON as `GET /v2/workers/territories/bootstrap` (state + paginated districts). Roles: `super_admin`, `central_support`, `state_leader`, `district_leader` (scoped). |
| `GET /v2/dashboard/territories/children?parent_id=` | Mandals (etc.) under a district — same JSON as workers territory children. |
...
| `raised` | Tickets filed by this worker (`worker_filed_ticket` audit). Includes all stages. Citizen name/phone shown. Tickets are territory auto-assigned at intake. `can_update_status` is `true` once this worker owns an accepted / force-assigned assignment. |
